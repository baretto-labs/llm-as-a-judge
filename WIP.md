...
7. **Ordre de reprise, arrêté le 2026-09-17.**
   1. **Arbitrage en attente de l'annotateur** (une phrase, mais il conditionne le gel) : sur `b08-011` et `b04-014`, l'omission d'une demande **facultative** a été imputée à `respect_consignes`. Soit il s'agit d'une énumération de griefs sur un item déjà FAIL — sans conséquence, seul le κ par contrôle en porte la trace ; soit la convention est récusée, et il faut réécrire ~10 items et la règle de `PROTOCOLE.md`. **À trancher à froid, pas en fin de séance.**
   2. **Instrumentation du coût — priorité absolue, seule tâche irréversible.** Rien ne mesure aujourd'hui la durée, la mémoire de pointe ni les jetons vus. Après `make train-14b`, ces chiffres sont perdus et il faut relancer des heures d'entraînement pour les obtenir. C'est le §8 du pré-enregistrement, et la seule preuve de l'affirmation « le fine-tuning n'est pas si coûteux ».
