...
5. **Étape en cours — relecture humaine du golden set** (`data/golden/test.jsonl`) et κ inter-annotateurs, qui sert de plafond à toutes les mesures. Toute correction d'étiquette passe par `generators/gen_batch_NN.py` puis `make regen`, jamais par le JSONL. Si une correction change un `verdict` ou un `cas`, les strates bougent : il faut rejouer `make split` et relire le delta.
