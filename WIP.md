...
4. ~~`make split`~~ : fait le 2026-09-14 en `SPLIT_SEED=3`. Golden set de 50 — 39 `CODE_ANALYSIS` / 5 `RAG_CONTEXT_RELEVANCE` / 6 `RAG_FAITHFULNESS`, cas 20/20/10 au point près, 26 PASS / 24 FAIL, 7 cas verbeux à défaut caché (14 %, plancher du protocole à 10 %). La graine 3 a été retenue sur 15 essais : le tirage étant glouton et groupé par famille, il ne peut pas couper une famille pour atteindre un quota, et la graine est le seul levier. Le défaut 42 était l'un des plus mauvais. **Attention : dans le `Makefile`, `SEED` désigne la liste des lots JSONL — la graine du tirage est `SPLIT_SEED`.**
5. **Étape en cours — reprise ciblée du corpus, bloquante avant tout entraînement.** L'audit du 2026-09-15 (`judge-finetune/AUDIT.md`, agent au contexte vierge, constats tous re-mesurés) montre que le corpus enseigne un raccourci de surface : la longueur du contenu évalué prédit le verdict à 75,3 % sur `CODE_ANALYSIS` et 84,0 % sur `RAG_FAITHFULNESS`, pour une base de ~53 %. Pire, le test de résistance au biais de verbosité du golden set est **séparable par la seule longueur** (verbeux+FAIL ≤ 1363 car., verbeux+PASS ≥ 1588) : un juge maximalement biaisé le réussit sans faute. Huit chantiers, dont 1 et 2 faits — détail et cibles dans `AUDIT.md`. Principe : allonger les FAIL, jamais raccourcir les PASS, une réponse verbeuse et fausse étant le cas difficile recherché.
6. **Relecture humaine du golden set — en pause** le temps de la reprise, inutile de valider des textes qui vont changer. Déjà relus : `b03-007` et `b04-015` (verdicts confirmés, journal dans `judge-finetune/data/REVUE_GOLDEN.md`) ; `b07-011` et `b10-015` présentés, réponse en attente. Protocole en aveugle : l'exemple est montré sans son étiquette, l'annotateur tranche, puis on compare. Toute correction passe par `generators/gen_batch_NN.py` puis `make regen`, jamais par le JSONL ; si un `verdict` ou un `cas` change, rejouer `make split`.
