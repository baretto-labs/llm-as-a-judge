...
10. **Journée du 2026-09-19 — troisième fuite, gel, et premier entraînement.**
    - **Fuite de mise en forme, trouvée par le balayage systématique** (`scripts/sweep_surface.py`, 22 traits testés d'un coup, null de permutation sur le **maximum**). Le nombre de titres `##` séparait les verdicts : **0 PASS / 24 FAIL** chez les non verbeux, puis les tableaux à **0/18**. Cause : j'écrivais les réponses fausses en documents soignés et les justes en prose directe, sur quatorze lots. **Point piquant pour le REX :** la littérature 2026 documente le biais *inverse* — les juges notent plus haut les réponses à titres —, donc le corpus enseignait à contredire l'a priori du modèle de base.
    - **Deux bugs dans l'instrument**, corrigés avant de s'y fier : la règle affichée ne disait pas son sens, et la recherche de seuil coupait à l'intérieur des ex æquo, ce qui gonflait `CODE_ANALYSIS` de 72,7 % à 74,0 %.
    - **Correctif en trois passes** : 8 réponses PASS structurées, 18 FAIL aplaties, 9 tableaux ajoutés. **Relecture intégrale à chaque passe** — c'est elle, et elle seule, qui a rattrapé `b05-002`, dont une phrase avait perdu son antécédent, et qui m'a empêché de supprimer le défaut semé de `b02-006`.
    - **Critère révisé et daté** dans `PREENREGISTREMENT.md` : « p ≥ 0,05 sur tous les traits » était une cible sans fin sur un corpus à auteur unique. Remplacé par **aucune cellule déterministe** — atteint —, la fuite résiduelle étant **mesurée et publiée** via une passe « format normalisé » ajoutée aux métriques secondaires.
    - **Corpus gelé**, splits régénérés (135/15/50), invariants intacts. **`make train-14b` lancé** : 16 couches, 400 itérations, 11,5 M paramètres entraînables sur 14,77 Md, pic 11,3 Go, perte 1,298 → 1,080 à mi-parcours.
    - **Relecture : 28 items sur 50**, accord verdict **27/28**, accord complet contrôle par contrôle **23/28**. **22 restants**, dont les 8 RAG à traiter en bloc final. **`b06-008` est présenté, réponse en attente.**
    - **Clarification de convention obtenue en séance (lecture B)** après deux désaccords d'attribution consécutifs : les trois contrôles sont indépendants et cumulatifs, `respect_consignes` ne couvre que les consignes explicites de la requête. Aucune modification du corpus. **Leçon : on suspend dès que la mesure devient douteuse, on ne l'accumule pas.**
    - **Une inattention commune consignée** (`b06-014`, `var` sur les paramètres de lambda) : un accord peut être un accord sur une erreur partagée, ce que le κ ne distingue pas.
11. **Décision de cadrage du REX** : la colonne vertébrale est la **construction du jeu d'évaluation**, pas le fine-tuning, qui devient un résultat parmi d'autres publié avec son intervalle. Trame dans `judge-finetune/REX_TRAME.md`. Échéance : **v0.1 pour le vendredi suivant**, l'utilisateur repassant devant ses associés.
