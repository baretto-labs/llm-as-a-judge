...
2. Générer les 200 exemples — **95/200 faits** (batch_01 : 5, puis 15 par lot). Reste 55 exemples de code (lots 08 à 11) avant de passer aux 50 RAG. Prompts pour déléguer la génération à des modèles tiers : `judge-finetune/PROMPT_GENERATION.md`. Méthode par lot : sonder les comportements par exécution, écrire les exemples à partir des sorties mesurées, valider, contrôler la dérive de distribution. Plan de composition et familles dans `data/PLAN_CORPUS.md`. Le 14B a été sondé sur 20 itérations : ~11 s/itération, pic 11,08 Go, soit ~1 h 15 pour 400 itérations.
